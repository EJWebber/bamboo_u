...
class WMGoalForm extends React.Component {
  ...
  dropdownChange = e => {
    ...
    this.setState({
      activity: this.props.WMGs.filter(
        goal => goal.activity === e.target.innerText
      )[0]
  ...
  handleSubmit = () => {
    this.state.times && this.state.activity
      ? this.submitWeeklyGoal()
  ...
  submitWeeklyGoal = () => {
    ...
    let goal = {
      ...
      wm_goal_id: this.state.activity,
      complete: false,
      number: this.state.times
    };
    API.postUserWMG(goal).then(this.props.addWMGoal);
    this.props.showForm();
  };
  ...
  render() {
    return (
      <div className="form">
        ...
        <br />
        {this.state.toggleWarning ? (
          <div>Please complete both fields</div>
        ) : null}
        <br />
        <Button.Group>
          <Button className="ui negative button" onClick={this.cancelSubmit}>
            Cancel
          </Button>
